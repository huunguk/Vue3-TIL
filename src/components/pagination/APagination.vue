<template>
	<div>
		<div class="items">
			<div v-for="item in paginatedItems" :key="item">{{ item }}</div>
		</div>
...
const itemsPerPage = ref(10) //페이지당 아이템 개수
const items = ref([...Array(80).keys()].map(i => i + 1)) //전체 아이템 리스트
...
// 현제 페이지 주변에 보여줄 페이지 수를 설정
const visiblePages = computed(() => {
	const pagesToShow = 5
	const pageDiff = Math.floor(pagesToShow / 2)
	let startPage = currentPage.value - pageDiff
	let endPage = currentPage.value + pageDiff
	if (startPage < 1) {
		startPage = 1
		endPage = pagesToShow
	}
...
function leftEndPage() {
	currentPage.value = 1
}

function rightEndPage() {
